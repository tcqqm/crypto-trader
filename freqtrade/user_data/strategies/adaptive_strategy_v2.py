"""
自适应交易策略 v2 — 基于 v8-stable 的 custom_exit 改进版
- 趋势跟踪（EMA交叉 + 多时间框架确认 + 多空对抗）
- 均值回归（RSI + BB 边界 + 1h 趋势过滤）
- custom_exit 按 enter_tag 分级出场（核心改进）
- Fear & Greed Index 宏观情绪过滤（live/dry-run 时生效）
- 回测 2 年：21 笔，81% 胜率，+1.77%，0.72% 回撤
"""
import logging
import time
...
class AdaptiveStrategyV2(IStrategy):
    """
    市场状态自适应策略 v2
    - ADX > 23 → 趋势跟踪（EMA交叉 + 冷却期）
    - ADX < 17 → 均值回归（RSI + 布林带边界）
    - custom_exit 按入场标签分级出场
    """
    ...
    # hyperopt 入场参数
    adx_trend_threshold = IntParameter(20, 30, default=23, space="buy")
    adx_range_threshold = IntParameter(15, 25, default=17, space="buy")
    rsi_entry_low = IntParameter(35, 50, default=50, space="buy")
    rsi_entry_high = IntParameter(60, 70, default=68, space="buy")
    signal_strength_min = DecimalParameter(0.10, 0.30, default=0.13, decimals=2, space="buy")
    atr_max_percentile = DecimalParameter(0.70, 0.90, default=0.78, decimals=2, space="buy")
    ema_cooldown = IntParameter(3, 16, default=8, space="buy")
    rsi_oversold_entry = IntParameter(22, 35, default=23, space="buy")

    # hyperopt 出场参数
    rsi_exit_trend = IntParameter(62, 75, default=71, space="sell")
    rsi_exit_revert = IntParameter(50, 65, default=61, space="sell")
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """出场信号留空，由 custom_exit 按 enter_tag 分级处理"""
        return dataframe
    ...
    def custom_exit(self, pair, trade, current_time, current_rate, current_profit, **kwargs):
        ...
        if trade.enter_tag == "trend_ema_cross":
            # 趋势出场：RSI过热 + 动量衰减
            if (last["rsi"] > self.rsi_exit_trend.value
                    and last["rsi"] < prev["rsi"]
                    and last["close"] > last["bb_middle"]):
                return "trend_rsi_exhaustion"

        elif trade.enter_tag == "revert_rsi_bb":
            # 均值回归出场：RSI回归中性 或 接近BB上轨
            if last["rsi"] > self.rsi_exit_revert.value:
                return "revert_rsi_target"
            if last["close"] >= last["bb_upper"] * 0.99:
                return "revert_bb_upper"
